...
from robo_vln_baselines.models.cma import CMANet
from robo_vln_baselines.models.seq2seq import Seq2SeqNet
...
class RoboDaggerTrainer(BaseRLTrainer):
    ...
    def _update_agent(
        ...
        if isinstance(aux_loss, torch.Tensor):
            ...
            loss = (action_loss.detach().item(), stop_loss.detach().item(), aux_loss)
    ...
    def train(self) -> None:
        ...
        if self.config.DAGGER.PRELOAD_LMDB_FEATURES:
            ...
            self.config.defrost()
            self.config.TASK_CONFIG.DATASET.SPLIT = self.config.DAGGER.COLLECT_DATA_SPLIT
            self.config.TASK_CONFIG.TASK.NDTW.SPLIT = self.config.DAGGER.COLLECT_DATA_SPLIT
            self.config.TASK_CONFIG.TASK.SDTW.SPLIT = self.config.DAGGER.COLLECT_DATA_SPLIT
            if self.config.DAGGER.P == 1.0:
                self.config.TASK_CONFIG.ENVIRONMENT.ITERATOR_OPTIONS.MAX_SCENE_REPEAT_STEPS = (
                    -1
                )
