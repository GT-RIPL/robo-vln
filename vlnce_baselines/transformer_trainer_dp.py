...
class TransformerTrainer(BaseRLTrainer):
    ...
    def _setup_actor_critic_agent(
        ...
        if config.TRANSFORMER.use:
            # self.actor_critic = TransformerEncDec(
            ...
            self.actor_critic = TransformerEncDec(
                observation_space=self.envs.observation_spaces[0],
                num_actions=self.envs.action_spaces[0],
                model_config=config,
                batch_size=self.config.DAGGER.BATCH_SIZE,
                gpus = [0,1,2]
            )
        self.actor_critic = nn.DataParallel(self.actor_critic, device_ids=[0,1,2])
        self.actor_critic.to(self.device)
        print("Let's use", torch.cuda.device_count(), "GPUs!")
        
        if isinstance(self.actor_critic, nn.DataParallel):
    ...
    def _update_agent(
        ...
        batch = (observations, prev_actions_list, prev_actions, not_done_masks)

        logits, _ = self.actor_critic(batch)
        # distribution = self.actor_critic.module.build_distribution(
        ...
        aux_loss = AuxLosses.reduce_parallel(aux_mask)
