...
from vlnce_baselines.models.transformer.transformer import ActionDecoderTransformer, HybridActionDecoder
...
class TransformerHybridPolicy(BaseTransformerPolicy):
    def __init__(
...
class TransformerEncDec(Net):
    ...
    def __init__(self, observation_space: Space, model_config: Config, num_actions, batch_size, gpus):
        ...
        self.sem_pooler = nn.Sequential(
            nn.AdaptiveAvgPool1d((16))
        )

        self.action_decoder = HybridActionDecoder(model_config.HYBRID_STATE_DECODER)
        ...
        self.state_compress = nn.Sequential(
            nn.Linear(
                model_config.TRANSFORMER.output_size*2 + self.prev_action_embedding.embedding_dim,
        ...
        if model_config.TRANSFORMER.split_gpus:
            self.embedding_layer.to(self.gpu0)
            self.ins_fc.to(self.gpu0)
            self.depth_encoder.to(self.gpu0)
            self.rgb_encoder.to(self.gpu0)
            self.visual_fc.to(self.gpu0)
            self.position_embedding_2d.to(self.gpu0)
            self.image_cm_encoder.to(self.gpu1)
            self.sem_map_encoder.to(self.gpu1)
            self.sem_pooler.to(self.gpu1)
            self.visual_sem_fc.to(self.gpu1)
            self.prev_action_embedding.to(self.gpu0)
            self.action_decoder.to(self.gpu0)
            self.progress_monitor.to(self.gpu0)
    ...
    def num_recurrent_layers(self):
        return self.action_decoder.num_recurrent_layers

    def _init_layers(self):
        nn.init.kaiming_normal_(self.progress_monitor.weight, nonlinearity="tanh")
        nn.init.constant_(self.progress_monitor.bias, 0)

    def single_forward(self, visual_embeddings, lang_encoding, progress, prev_actions, rnn_hidden_states, masks):
        r"""
        ...
        visual_enc_out = self.image_cm_encoder(rgbd_out.to(self.gpu1), ins_enc_out.to(self.gpu1), None, enc_mask.to(self.gpu1), pos_embed_out.to(self.gpu1))
        # visual_enc_out = self.visual_pooler(visual_enc_out.permute(0,2,1))
        ...
        sem_enc_out = self.sem_map_encoder(sem_map.to(self.gpu1), ins_enc_out.to(self.gpu1), None, enc_mask.to(self.gpu1))
        sem_enc_out = self.sem_pooler(sem_enc_out.permute(0,2,1)).permute(0,2,1)

        x = torch.cat((visual_enc_out, sem_enc_out), dim=-1)
        x = F.relu(self.visual_sem_fc(x))
        ...
        if self.model_config.TRANSFORMER.use_prev_action:
            ...
            # x=self.state_compress(x)

        

        out, rnn_hidden_states = self.action_decoder(x.to(self.gpu0), ins_enc_out.to(self.gpu0), None, enc_mask.to(self.gpu0), rnn_hidden_states, 
                                    masks, prev_actions_embedding)
        return out, rnn_hidden_states

    def seq_forward(self, visual_embeddings, lang_encoding, progress, prev_actions, rnn_hidden_states, masks):
        rgb_embedding, depth_embedding, sem_map = visual_embeddings
        ...
        for i in range(max_len):
            prev_actions_single       = prev_actions[:,i].unsqueeze(1)
            masks_single              = masks[:,i].unsqueeze(1)
            ...
            out, rnn_hidden_states = self.single_forward(visual_embeddings, lang_encoding, progress, prev_actions_single, rnn_hidden_states, masks_single)
            out = out.unsqueeze(1)
            output.append(out)
            del out
    ...
    def forward(self, observations, rnn_hidden_states, prev_actions, masks):
        ...
        if batch_dim == num_process_dim:
            ...
            masks = masks.view(-1)
            output, rnn_hidden_states = self.single_forward(visual_embeddings, lang_encoding, progress, prev_actions,rnn_hidden_states, masks)
            ...
            output, rnn_hidden_states = self.seq_forward(visual_embeddings, lang_encoding, progress, prev_actions, rnn_hidden_states, masks)
            if self.model_config.PROGRESS_MONITOR.use and AuxLosses.is_active():
                progress_hat = torch.tanh(self.progress_monitor(output))
                progress_loss = F.mse_loss(
                    progress_hat.squeeze(1), progress, reduction="none"
