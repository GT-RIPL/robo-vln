...
class TransformerSemPolicyMP(BaseTransformerPolicy):
    def __init__(
...
class TransformerEncDec(Net):
    ...
    def __init__(self, observation_space: Space, model_config: Config, num_actions, batch_size, gpus):
        ...
        if model_config.TRANSFORMER.split_gpus:
            ...
            self.action_decoder.to(self.gpu2)
            self.progress_monitor.to(self.gpu2)
    ...
    def single_forward(self, visual_embeddings, lang_encoding, progress, prev_actions, prev_actions_list, masks, prev_action_pad_mask=None):
        ...
        sem_enc_out, _ = self.sem_map_encoder(sem_map.to(self.gpu1), ins_enc_out.to(self.gpu1), None, enc_mask.to(self.gpu1))
        ...
        out = self.action_decoder(prev_action_seq.to(self.gpu2), ins_enc_out.to(self.gpu2), visual_semantic_enc.to(self.gpu2), enc_att_mask_w=enc_mask.to(self.gpu2), 
                                    enc_att_mask_i=None, device=self.device, pos_embed= pos_embed_out.to(self.gpu2))
    ...
    def forward(self, observations, prev_actions_list, prev_actions, masks):
        ...
        if batch_dim == num_process_dim:
            ...
            if self.model_config.PROGRESS_MONITOR.use and AuxLosses.is_active():
                progress_hat = torch.tanh(self.progress_monitor(output))
                progress_loss = F.mse_loss(
                    progress_hat.squeeze(1), progress.to(self.gpu2), reduction="none"
                )
                AuxLosses.register_loss(
                    "progress_monitor",
                    progress_loss,
