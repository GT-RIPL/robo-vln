...
class TransformerPolicyDDP(BaseTransformerPolicy):
    def __init__(
...
class TransformerEncDec(Net):
    ...
    def __init__(self, observation_space: Space, model_config: Config, num_actions, batch_size, gpus):
        ...
        if model_config.split_gpus:
            self.embedding_layer.to(self.gpu0)
            self.instruction_encoder.to(self.gpu0)
    ...
    def single_forward(self, visual_embeddings, lang_encoding, progress, prev_actions, prev_actions_list, masks, prev_action_pad_mask=None):
        ...
        visual_enc_out = self.image_cm_encoder(rgb_out.to(self.gpu1), ins_enc_out.to(self.gpu1), None, enc_mask.to(self.gpu1), pos_embed_out.to(self.gpu1)) 
        ...
        out = self.action_decoder(
                    prev_action_seq.to(self.gpu2), ins_enc_out.to(self.gpu2), visual_enc_out.to(self.gpu2), enc_att_mask_w=enc_mask.to(self.gpu2), 
                    enc_att_mask_i=None, device=self.gpu2, action_pad_mask = prev_action_pad_mask.to(self.gpu2))
    ...
    def forward(self, observations, prev_actions_list, prev_actions, masks):
        ...
        ins_enc_out = self.instruction_encoder(value, attention_mask=attn_mask, attention_weights=None, device=self.gpu0) 
