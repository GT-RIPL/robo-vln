...
class Seq2Seq_Lang_Attn(BasePolicy):
    def __init__(
...
class Seq2SeqNet(Net):
    ...
    def __init__(self, observation_space: Space, model_config: Config, num_actions):
        ...
        self.state_encoder = Attn_DecoderSequence(
            attn_model = 'general',
    ...
    def forward(self, observations, rnn_hidden_states, prev_actions, masks):
        ...
        """
        lengths = (observations["instruction"].long() != 0.0).long().sum(dim=1)
        ...
        if len(rnn_hidden_states)==0:    
            rnn_hidden_states = encoder_hidden
            
        x, rnn_hidden_states, attn_weights = self.state_encoder(x, rnn_hidden_states, encoder_hidden, encoder_output, masks, lengths, device)
