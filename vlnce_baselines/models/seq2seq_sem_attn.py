...
class Seq2Seq_Sem_Attn_Net(Net):
    ...
    def __init__(self, observation_space: Space, model_config: Config, num_actions):
        ...
        self.model_config = model_config
         ...
         # Init the Semantic Attention encoder
        ...
        if model_config.split_gpus:
            # nn.DataParallel(self.sem_map_attn_encoder, device_ids=[0,2])
            self.sem_map_attn_encoder.to(device)
            self.instruction_encoder.to(device)
            # self.depth_encoder.to(device)
            # self.rgb_encoder.to(device)
            self.state_encoder.to(device)
            # nn.DataParallel(self.instruction_encoder, device_ids=[1,2]).to(device)
            # nn.DataParallel(self.depth_encoder, device_ids=[1,2]).to(device)
            # nn.DataParallel(self.rgb_encoder, device_ids=[1,2]).to(device)
            # nn.DataParallel(self.state_encoder, device_ids=[1,2]).to(device)
    ...
    def forward(self, observations, rnn_hidden_states, prev_actions, masks):
        ...
        # obs_sem_map ={}
        # # obs_sem_map['ego_sem_map'] = observations['ego_sem_map'].cuda(2)
        # obs_sem_map['ego_sem_map'] = observations['ego_sem_map'].
        sem_map_attn_embedding = self.sem_map_attn_encoder(observations)
        # sem_map_attn_embedding = sem_map_attn_embedding.cuda(1)
        ...
        x = torch.cat([instruction_embedding, depth_embedding, rgb_embedding, sem_map_attn_embedding], dim=1)
