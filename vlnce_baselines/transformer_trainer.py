...
from vlnce_baselines.models.transformer_policy_ddp import TransformerPolicyDDP
from vlnce_baselines.models.transformer_policy_update import TransformerEncDec
from vlnce_baselines.models.transformer_semantics_policy import TransformerSemPolicy
from vlnce_baselines.models.transformer_semantics_policy_mp import TransformerSemPolicyMP
...
class TransformerTrainer(BaseRLTrainer):
    ...
    def _setup_actor_critic_agent(
        ...
        # self.scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer, self.config.MODEL.TRANSFORMER.lr_drop, gamma=0.8)
        self.scheduler = torch.optim.lr_scheduler.CyclicLR(self.optimizer, base_lr=2e-6, max_lr=1e-4, step_size_up=1000,step_size_down=30000, cycle_momentum=False)
        ...
        with tqdm.tqdm(total=self.config.DAGGER.UPDATE_SIZE) as pbar, lmdb.open(
            ...
            while collected_eps < self.config.DAGGER.UPDATE_SIZE:
                ...
                for i in range(self.envs.num_envs):
                    ...
                    # Only delete if semantic is in observations batch    
                    del observations[i]["semantic"]
    ...
    def _update_agent(
        ...
        # if self.config.MODEL.TRANSFORMER.split_gpus:
        #     corrected_actions = corrected_actions.cuda(2)
        #     weights = weights.cuda(2)
        ...
        del logits, loss, action_loss, aux_loss
                ...
                for epoch in tqdm.trange(self.config.DAGGER.EPOCHS):
                    for batch in tqdm.tqdm(
                        ...
                        try:
                            ...
                            #     observations_batch,
                            #     prev_actions_batch.cuda(),
                            #     not_done_masks.cuda(),
                            #     corrected_actions_batch.cuda(),
                            #     weights_batch.cuda(),
                            # )
                            loss, action_loss, aux_loss = self._update_agent(
                                observations_batch,
                        ...
                        # writer.add_scalar(
                        #     f"Learning_rate_vs_stepsize_it_{dagger_it}", lr_step, step_id
                        # )
                        # writer.add_scalar(
                        #     f"Loss_vs_log_learning rate_it_{dagger_it}", loss, lr_step
                        # )
    ...
    def _eval_checkpoint(
        ...
        config.defrost()
        config.TASK_CONFIG.SET.SPLIT = config.EVAL.SPLIT
        ...
        while (
            ...
            for i in range(self.envs.num_envs):
                ...
                k+=1
                print("Numer of episdes completed: ",k)
                prev_actions_list =[]
